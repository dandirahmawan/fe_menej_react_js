...
class filter extends React.Component{
    ...
    componentDidMount(){
        ...
        this.props.dataDocFile.map(dt => {
            console.log(dt)
            var idx = arr.indexOf(dt.modulId)
            if(idx == -1 && dt.isDelete != "Y"){
                arr.push(dt.modulId)
                this.state.dataFilter.push(dt.moduleName)
                this.setState({
    ...
    render(){
        ...
        return(
            <div className="main-border" style={{background: "#FFF", height: "200px", width: "200px", position: "absolute", marginTop: "20px", borderRadius: "3px"}}>
                <div className="main-border-bottom bold" id="hd-filter" style={{padding: "5px", fontSize: "12px", background: "#f5f5f5"}}>
                    Filter
                </div>
